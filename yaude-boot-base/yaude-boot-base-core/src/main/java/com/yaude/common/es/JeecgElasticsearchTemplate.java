...
/**
 * 關于 ElasticSearch 的一些方法（創建索引、添加數據、查詢等）
 *
 * @author sunjianlei
 */
@Slf4j
...
public class JeecgElasticsearchTemplate {
        ...
        if (StringUtils.isNotEmpty(baseUrl)) {
            ...
            if (checkEnabled) {
                try {
                    ...
                    log.info("ElasticSearch 服務連接成功");
                    log.info("ElasticSearch version: " + this.version);
                } catch (Exception e) {
                    this.version = "";
                    log.warn("ElasticSearch 服務連接失敗，原因：配置未通過。可能是BaseURL未配置或配置有誤，也可能是Elasticsearch服務未啟動。接下來將會拒絕執行任何方法！");
                }
            }
        }
    }
    ...
    /**
     * 獲取 Elasticsearch 的版本號信息，失敗返回null
     */
    ...
    /**
     * cat 查詢ElasticSearch系統數據，返回json
     */
     ...
     * 查詢所有索引
     * <p>
     * 查詢地址：GET http://{baseUrl}/_cat/indices
     */
     ...
     * 查詢單個索引
     * <p>
     * 查詢地址：GET http://{baseUrl}/_cat/indices/{indexName}
     */
    ...
    /**
     * 根據ID獲取索引數據，未查詢到返回null
     * <p>
     * 查詢地址：GET http://{baseUrl}/{indexName}/{typeName}/{dataId}
     *
     * @param indexName 索引名稱
     * @param typeName  type，一個任意字符串，用于分類
     * @param dataId    數據id
     * @return
     */
     ...
     * 創建索引
     * <p>
     * 查詢地址：PUT http://{baseUrl}/{indexName}
     */
        ...
        */
        try {
            ...
            if (HttpStatus.BAD_REQUEST == ex.getStatusCode()) {
                log.warn("索引創建失敗：" + indexName + " 已存在，無需再創建");
            } else {
                ex.printStackTrace();
            }
        }
     ...
     * 刪除索引
     * <p>
     * 查詢地址：DELETE http://{baseUrl}/{indexName}
     */
    public boolean removeIndex(String indexName) {
        ...
        try {
            ...
            if (HttpStatus.NOT_FOUND == ex.getStatusCode()) {
                log.warn("索引刪除失敗：" + indexName + " 不存在，無需刪除");
            } else {
                ex.printStackTrace();
            }
        }
     ...
     * 保存或修改索引數據
     * <p>
     * 查詢地址：PUT http://{baseUrl}/{indexName}/{typeName}/{dataId}
     *
     * @param indexName 索引名稱
     * @param typeName  type，一個任意字符串，用于分類
     * @param dataId    數據id
       ...
       "createIndexA2": {
        ...
        try {
            ...
            for (String key : keys) {
                ...
                //2、剔除上傳控件值(會導致ES同步失敗，報異常failed to parse field [ge_pic] of type [text] )
                if (oConvertUtils.isNotEmpty(value) && value.indexOf("[{")!=-1) {
                    emptyKeys.add(key);
                    log.info("-------剔除上傳控件字段------------key: "+ key);
                }
            }
     ...
     * @param indexName 索引名稱
     * @param typeName  type，一個任意字符串，用于分類
     * @param dataList  要存儲的數據數組，每行數據必須包含id
     * @return
     */
     ...
     * 刪除索引數據
     * <p>
     * 請求地址：DELETE http://{baseUrl}/{indexName}/{typeName}/{dataId}
     */
     ...
     * 查詢數據
     * <p>
     * 請求地址：POST http://{baseUrl}/{indexName}/{typeName}/_search
     */
    ...
    /**
     * @param _source （源濾波器）指定返回的字段，傳null返回所有字段
     * @param query
     * @param from    從第幾條數據開始
     * @param size    返回條目數
     * @return { "query": query }
    ...
    /**
     * @param field 要查詢的字段
     * @param args  查詢參數，參考： *哈哈* OR *噠* NOT *呵* OR *啊*
     * @return
     */
     ...
     * @param min        最小值
     * @param max        最大值
     * @param containMin 范圍內是否包含最小值
     * @param containMax 范圍內是否包含最大值
