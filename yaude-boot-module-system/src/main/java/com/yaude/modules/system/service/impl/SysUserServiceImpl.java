...
public class SysUserServiceImpl extends ServiceImpl<SysUserMapper, SysUser> implements ISysUserService {
	...
	public Result<?> resetPassword(String username, String oldpassword, String newpassword, String confirmpassword) {
		...
		if (!user.getPassword().equals(passwordEncode)) {
			return Result.error("舊密碼輸入錯誤!");
		}
		if (oConvertUtils.isEmpty(newpassword)) {
			return Result.error("新密碼不允許為空!");
		}
		if (!newpassword.equals(confirmpassword)) {
			return Result.error("兩次輸入密碼不一致!");
		}
		String password = PasswordUtil.encrypt(username, newpassword, user.getSalt());
		this.userMapper.update(new SysUser().setPassword(password), new LambdaQueryWrapper<SysUser>().eq(SysUser::getId, user.getId()));
		return Result.ok("密碼重置成功!");
	}
...
//		this.save(user);  //保存角色的時候已經添加過一次了
	...
	public void editUserWithDepart(SysUser user, String departs) {
		this.updateById(user);  //更新角色的時候已經更新了一次了，可以再跟新一次
		String[] arr = {};
	...
	public Result<?> checkUserIsEffective(SysUser sysUser) {
		...
		//情況1：根據用戶信息查詢，該用戶不存在
		if (sysUser == null) {
			result.error500("該用戶不存在，請註冊");
			baseCommonService.addLog("用戶登錄失敗，用戶不存在！", CommonConstant.LOG_TYPE_1, null);
			return result;
		}
		//情況2：根據用戶信息查詢，該用戶已註銷
		//update-begin---author:王帥   Date:20200601  for：if條件永遠為falsebug------------
		if (CommonConstant.DEL_FLAG_1.equals(sysUser.getDelFlag())) {
			//update-end---author:王帥   Date:20200601  for：if條件永遠為falsebug------------
			baseCommonService.addLog("用戶登錄失敗，用戶名:" + sysUser.getUsername() + "已註銷！", CommonConstant.LOG_TYPE_1, null);
			result.error500("該用戶已註銷");
			return result;
		}
		//情況3：根據用戶信息查詢，該用戶已凍結
		if (CommonConstant.USER_FREEZE.equals(sysUser.getStatus())) {
			baseCommonService.addLog("用戶登錄失敗，用戶名:" + sysUser.getUsername() + "已凍結！", CommonConstant.LOG_TYPE_1, null);
			result.error500("該用戶已凍結");
			return result;
		}
		return result;
	}
	...
	public boolean removeLogicDeleted(List<String> userIds) {
		...
		try {
			...
			log.error("同步刪除第三方App的用戶失敗：", e);
		}
		//5. 刪除第三方用戶表（因為第4步需要用到第三方用戶表，所以在他之後刪）
