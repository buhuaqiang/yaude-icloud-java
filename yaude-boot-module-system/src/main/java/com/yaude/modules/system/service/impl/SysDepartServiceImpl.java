...
import com.yaude.modules.system.util.FindsDepartsChildrenUtil;
...
public class SysDepartServiceImpl extends ServiceImpl<SysDepartMapper, SysDepart> implements ISysDepartService {
	...
	/**
	 * saveDepartData 對應 add 保存用戶在頁面添加的新的部門對象數據
	 */
	@Override
	@Transactional
	public void saveDepartData(SysDepart sysDepart, String username) {
		if (sysDepart != null && username != null) {
			...
			sysDepart.setId(s);
			// 先判斷該對象有無父級ID,有則意味著不是最高級,否則意味著是最高級
			// 獲取父級ID
			String parentId = sysDepart.getParentId();
			//update-begin--Author:baihailong  Date:20191209 for：部門編碼規則生成器做成公用配置
			JSONObject formData = new JSONObject();
			formData.put("parentId",parentId);
			String[] codeArray = (String[]) FillRuleUtil.executeRule(FillRuleConstant.DEPART,formData);
			//update-end--Author:baihailong  Date:20191209 for：部門編碼規則生成器做成公用配置
	...
	/**
	 * saveDepartData 的調用方法,生成部門編碼和部門類型（作廢邏輯）
	 * @deprecated
	 * @param parentId
	 * @return
	 */
	private String[] generateOrgCode(String parentId) {	
		//update-begin--Author:Steve  Date:20190201 for：組織機構添加數據代碼調整
				LambdaQueryWrapper<SysDepart> query = new LambdaQueryWrapper<SysDepart>();
				...
				strArray[1] = orgType;
				return strArray;
		//update-end--Author:Steve  Date:20190201 for：組織機構添加數據代碼調整
	} 

	
	/**
	 * removeDepartDataById 對應 delete方法 根據ID刪除相關部門數據
	 * 
	 */
	/*
	 * @Override
	 ...
	 * System.out.println("要刪除的ID 為=============================>>>>>"+id); boolean
	 * flag = this.removeById(id); return flag; }
	 */

	/**
	 * updateDepartDataById 對應 edit 根據部門主鍵來更新對應的部門數據
	 */
	@Override
	@Transactional
	...
	public List<SysDepartTreeModel> searhBy(String keyWord,String myDeptSearch,String departIds) {
		...
		//myDeptSearch不為空時為我的部門搜索，只搜索所負責部門
		if(!StringUtil.isNullOrEmpty(myDeptSearch)){
			//departIds 為空普通用戶或沒有管理部門
		...
		//update-begin--Author:huangzhilin  Date:20140417 for：[bugfree號]組織機構搜索回顯優化--------------------
	    ...
	    //update-end--Author:huangzhilin  Date:20140417 for：[bugfree號]組織機構搜索功回顯優化----------------------
				newList.add(model);
			}
			return newList;
		}
	...
	public List<SysDepartTreeModel> queryTreeListByPid(String parentId) {
		...
		for (int i = 0; i < list.size(); i++) {
            ...
            //TODO 異步樹加載key拼接__+時間戳,以便于每次展開節點會刷新數據
